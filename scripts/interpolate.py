...
def precompute(xs, prime):
    ...
    # Input: Elements x_1, x_2, ..., x_n with i != j => x_i != x_j
    # Output: Square matrix (m_1.1,...,m_n.n)
    n = len(xs)
...
def interpolate(ys, matrix, prime):
    # Algorithm 3: Interpolation after precomputation
    # Input: Square matrix (m_1.1,...,m_n.n)
    # Output: Coefficients c_0,...,c_n-1
    n = len(ys)
