...
def simulate_decoyauth_handshake():
	...
	log(DEBUG, f"[STA]    EA = {int(EA.x):X}")
	log(DEBUG, f"[STA]         {int(EA.y):X}")
	...
	coeff_u = weaver(u_values, matrix, secp256r1_p)
	coeff_v = weaver(v_values, matrix, secp256r1_p)
	...
	for i in range(NUM_KEYS):
		...
		if i == CLIENT_INDEX:
			...
			log(DEBUG, f"[STA]    Decoded EB = {EBi_x:X}")
			log(DEBUG, f"[STA]                 {EBi_y:X}")
