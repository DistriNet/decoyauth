						 ...
						 bool *ret_use_pk)
{
	...
	wpa_s->password = os_malloc(strlen(password) + 1);
	if (!wpa_s->password) {
		wpa_dbg(wpa_s, MSG_INFO, "SAE: Failed to allocate memory for password copy");
		goto fail;
	}
	os_memcpy(wpa_s->password, password, strlen(password) + 1);
			...
			int external, const u8 *sa, int *ie_offset)
{
	...
	if (auth_transaction == 1) {
		...
		wpa_printf(MSG_DEBUG, "SME: USING PASSWORD: %s", wpa_s->password);

		res = sae_ap_parse_commit(&wpa_s->sme.sae, (u8 *) wpa_s->password, os_strlen(wpa_s->password), data, len, NULL, NULL,
				       groups, status_code ==
				       WLAN_STATUS_SAE_HASH_TO_ELEMENT ||
				       status_code == WLAN_STATUS_SAE_PK,
				       ie_offset);
		os_free(wpa_s->password);
